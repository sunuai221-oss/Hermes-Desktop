...
wsl.exe -d %HERMES_WSL_DISTRO% -e bash -lc "if [ -n \"$HERMES_WSL_HOME\" ]; then export HERMES_HOME=\"$HERMES_WSL_HOME\"; fi; export PORT=%HERMES_GATEWAY_PORT%; export PATH=\"$HOME/.local/bin:$PATH\"; pidfile=\"$HERMES_HOME/gateway.pid\"; if [ -n \"$HERMES_HOME\" ] && [ -f \"$pidfile\" ]; then pid=$(grep -o '[0-9]\+' \"$pidfile\" | head -n 1); if [ -n \"$pid\" ] && kill -0 \"$pid\" 2>/dev/null; then echo \"[Hermes Gateway] Existing gateway already running with PID $pid.\"; exit 0; fi; echo \"[Hermes Gateway] Removing stale gateway.pid ($pid).\"; rm -f \"$pidfile\"; fi; exec hermes gateway run"
echo.
